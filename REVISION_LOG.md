...
- Added explicit cost bounds for all six framework algorithms. HDR insertion uses tree height $L$, fanout $f$, and level dimensionalities $d_l$, while deletion and refresh account for path traversal and stored-state maintenance; the four join-update cases use compositional kNN, RkNN, row-update, and index-update costs.
...
- Removed the $O(f\sum_l d_l)$ term from deletion. HDR-Tree already stores each query-to-center distance, so radius tightening uses stored scalar distances rather than new $d_l$-dimensional calculations. This inherited mechanism is not presented as a contribution of this work.
...
- Expanded the query-insertion case into two separate correctness obligations: adding the exact join row and inserting the query into HDR-Tree for future RkNN search. Both depend on the preceding Delta-Tree kNN search, after which the two writes are independent.
- Restored the full dependency rhythm of Cases 1, 3, and 4: each now states the table and index states affected, connects them to the future kNN/RkNN search paths, derives the required operation order, and only then presents the localized-versus-full-recomputation cost.
- Separated the Case 1 table-maintenance obligation from its later kNN-computation prerequisite, strengthened the Case 3 savings statement by identifying the eliminated $n_U T_{\knn}$ term, and reserved ``index components'' for maintained state versus ``search paths'' for invoked computation.
- Recentered the Abstract and Introduction on bidirectional dual-index search and coupled maintenance of the materialized join table and dual-index state. Removed the fourth challenge and the locate-and-repair contribution framing, promoted the dynamic HDR-Tree extension, and synchronized the four contribution statements and section roadmap.
- Removed the unexplained ``exact maintenance before optional approximate serving layers'' claim from the Abstract and rewrote dimensionality selection as mathematical estimation of a near-optimal dimension rather than a heuristic algorithm.
- Verified Table~2 numerically: the fastest non-Dual-Tree baseline yields per-dataset speedups of 1.5280--2.6509$\times$ for \dualtreeopt, averaging 1.8793$\times$. Updated the Abstract, Introduction, and experiment prose to report 1.88$\times$ average and 2.65$\times$ maximum under this explicit baseline scope.
- Tightened the Abstract back to journal-summary granularity: restored the original opening, summarized Dual-Tree as a dynamically maintainable dual index for bidirectional kNN search, removed implementation-level HDR-Tree and cost-model details, and retained only the maximum 2.65$\times$ result over the fastest baseline.
- Closed the Abstract's framework argument by combining bidirectional search and coordinated index/join-table maintenance into one result-oriented statement that explicitly delivers exact fully dynamic kNN join processing in high-dimensional spaces.
- Expanded the Abstract's core framework statement into an explicit four-step chain: Dual-Tree contains a dual index; the index supports query-to-reference kNN and reference-to-query RkNN search in high dimensions; it supports updates on either set; and a dual-maintenance strategy jointly maintains the index and materialized join table to realize exact fully dynamic processing.
- Generalized the Abstract's performance statement from the specific Dual-Tree-opt variant to the proposed Dual-Tree method family while retaining the verified maximum 2.65$\times$ speedup.
- Refined the Introduction's solution narrative by separating the Delta-Tree kNN and HDR-Tree RkNN directions, making affected-row localization and avoided full recomputation explicit, and describing dynamic maintenance at the dual-index-system level. Recast dimensionality selection as a rigorous, theoretically justified optimization over pruning power and distance-computation cost. Merged the first two contributions into a dynamic dual-index framework contribution, strengthened the Dual-Tree leaf-level pruning contribution, and retained only the maximum 2.65$\times$ evaluation result.
- Expanded RkNN as ``reverse kNN'' in both the standalone Abstract and its first main-text occurrence. Rewrote the framework contribution in active voice to foreground our dynamic index mechanisms and maintenance strategy, removed the unelaborated count of four strategies, and made the cost model's rigorous derivation and theoretically grounded approximation to the optimal pruning dimensionality the main result.
- Reworked both Introduction descriptions of fine-grained pruning into an explicit dependency and causal chain: the dimension-selection model now serves the new point-wise pruning stage, models its pruning-power versus distance-computation-cost trade-off, and yields a theoretically grounded approximation to the optimal pruning dimensionality by mathematically minimizing the resulting expected cost.
- Generalized the dimension-selection model's presentation so it is used by the Dual-Tree pruning check without being framed as exclusive to that framework. Shortened the main Introduction account, expanded the contribution account, and made the model itself the subject that mathematically derives the theoretically grounded approximation to the optimal pruning dimensionality.
- Added a deliberately non-exclusive link between the fine-grained pruning contribution and the cost-aware model: the model is applied here to select the point-to-point check's dimensionality, without being described as designed only for Dual-Tree.
