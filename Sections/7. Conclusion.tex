...
\noindent\textbf{\textit{Step 1.}} For each sampled data point, compute the average value of the $dist_{kNN}$ values of all its RkNN members. Then, divide the sampled data points into groups where the average $dist_{kNN}$ values of RkNN members vary by no more than a small threshold ratio $\Delta$. For arbitrary sampled points $q_1$ and $q_2$ in the same group, the following relationship holds:
\begin{flalign}
...
\noindent\textbf{\textit{Step 2.}}
For each sampled data point in each group, the $dist_{kNN}$ values of the data points visited at the leaf nodes of the \hdrtree part of the \dualtree during its RkNN search process are divided into different sets. For arbitrary two $dist_{kNN1}$ and $dist_{kNN2}$ values in the same set, the following relationship holds:
\begin{flalign}
...
\noindent\textbf{2). Derive the weight values \bm{$p_m$} and the parameters \bm{$\sigma_m$} and \bm{$\mu_m$} of each sub-model.}
Similar to the kNN search case, the EM algorithm is then applied to estimate the weight $p_m$ and the parameters $\mu_m$ and $\sigma_m$ for each sub-model using the sampled $dist^2(q,v)/b_p^2$ values collected during RkNN search.

This appendix describes how the parameters of $f_{dist^2(q,v)/b^2_p}$ are estimated from initial-state samples under the stable-phase assumption that the data distribution changes slowly during the update process.
