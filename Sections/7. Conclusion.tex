...
\begin{lemma}
    ...
    \begin{enumerate}
        ...
        \begin{flalign}
        f_{Z_k}(x)=0, x\notin[a_k,b_k]\IEEEyesnumber \label{equ:pdf_not_in_ab}
        \end{flalign}
        \item the nature of $f_{Z_k}(x)$ in the interval $[a_k,b_k]$ leads to the validation of the following relation:
    \end{enumerate}    
        \begin{flalign}
            \kappa_1 \leq \frac{E(Z^2_k)}{E((Z'_k)^2)}\leq \kappa_2, \quad 0<\iota\leq \frac{Var(Z^2_k)}{Var((Z'_k)^2)}\IEEEyesnumber \label{equ:expectation_variance_appro}
            \end{flalign}    

\noindent then, the conclusion below can be drawn that:
\begin{flalign}
...
\end{flalign}
Given $E(Z^{'2}_k)$ and $Var(Z^{'2}_k)$, two variables composing the right side of equation \ref{equ:Y_k_TO_Z_k} that $\sum^D_{k=1}E(|Z^2_k-E(Z^2_k)|^{2+\delta})$ and $(\sum^D_{k=1}Var(Z^2_k))^{\frac{\delta}{2}}$ can be analyzed to progress the proof.

For $(\sum^D_{k=1}Var(Z^2_k))^{\frac{\delta}{2}}$, because of equation \ref{equ:z_k_variance} and the condition stated in expression \ref{equ:expectation_variance_appro}, a lower bound of $(\sum^D_{k=1}Var(Z^2_k))^{\frac{\delta}{2}}$ can be derived that:
\begin{flalign}
&(\sum^D_{k=1}Var(Z^2_k))^{\frac{\delta}{2}}\IEEEnonumber\\
\geq&(\iota\sum^D_{k=1}\frac{(4a^2_k + 7a_kb_k + 4b^2_k)(b_k - a_k)^2}{45})^{\frac{\delta}{2}}\IEEEyesnumber
...
\leq &\max\{|b^2_k-\frac{\kappa_1(a^2_k+a_kb_k+b^2_k)}{3}|, \IEEEnonumber\\
&\quad\quad\;\; |a^2_k-\frac{\kappa_2(a^2_k+a_kb_k+b^2_k)}{3}|\}   \IEEEyesnumber \label{equ:two_upperbound}\\
...
\begin{flalign}
&\frac{\sum^D_{k=1}E(|Y_k|^{2+\delta})}{\sqrt{\sum^D_{k=1}Var(Y_k)}^{2+\delta}}\IEEEnonumber \\
...
\end{flalign}
\begin{proof}
    \begin{flalign}    &\frac{\sum^D_{k=1}\int_{|x|>\epsilon s_D}|x|^2f_{Y_k}(x)dx}{s^2_D}\leq\IEEEnonumber \\
 &\frac{\sum^D_{k=1}\int_{|x|>\epsilon s_D}|x|^{2+\delta}f_{Y_k}(x)dx}{\epsilon^{\delta}s^{2+\delta}_D}\leq  \IEEEnonumber \\
&\frac{\sum^D_{k=1}\int_{\infty}|x|^{2+\delta}f_{Y_{k}}(x)dx}{\epsilon^{\delta}s^{2+\delta}_D}=\frac{\sum^D_{k=1}E(|Y_k|^{2+\delta})}{\epsilon^{\delta}s^{2+\delta}_D}
    \end{flalign}
\end{proof}
...
For a random variable $Y$, its characteristic function is
\[
\psi_Y(t)=E(e^{itY}),
\]
where $E$ denotes the expected value and $i$ is the imaginary unit. In particular, the characteristic function of the standard normal distribution $\mathcal{N}(0,1)$ is
\[
...
What needs to be proved is that the proposition below holds approximately:
\begin{flalign}
dist^2(q_{\mathcal{O}^i},v^{j}_{q_{\mathcal{O}^i}})/b_{p}^2(q_{\mathcal{O}^i},j) \sim \mathcal{N}(\mu,\,\sigma^2) \IEEEyesnumber  \label{equ:normal_distribution_1}
\end{flalign}
In the proposition above, $q_{\mathcal{O}^i}$ represents an arbitrary data point in the $i$-th subspace of $\mathcal{O}$, $v^{j}_{q_{\mathcal{O}^i}}$ represents a random data point visited at the leaf nodes of \dualtree encountering the $j$-th pruning bound or bound kernel of $q_{\mathcal{O}^i}$, and $b_p(q_{\mathcal{O}^i},j)$ represents the $j$-th pruning bound or bound kernel value of $q_{\mathcal{O}^i}$. Note that although $q_{\mathcal{O}^i}$ is an arbitrary data point in $O^{i}$, it is treated as a fixed point when constructing the random variable $dist^2(q_{\mathcal{O}^i},v^{j}_{q_{\mathcal{O}^i}})/b_{p}^2(q_{\mathcal{O}^i},j)$. Therefore, $b_p(q_{\mathcal{O}^i},j)$, the $j$-th pruning bound or bound kernel value of $q_{\mathcal{O}^i}$, which is considered as a fixed point, is also considered as a constant value. Consequentely, Consequently, it suffices to prove the following normalized form:
\begin{flalign}
dist^2(q_{\mathcal{O}^i},v^{j}_{q_{\mathcal{O}^i}}) \sim \mathcal{N}(\mu,\,\sigma^2), \quad \text{approximately} \IEEEyesnumber \label{equ:normal_distribution_2}
\end{flalign}
...
\end{flalign}
where $k_{PC}$ and $D_{PC}$ respectively represents the $k$-th and $D$-th (last) principal component dimension. For notational simplicity in what follows, we use $Z_k$ to denote $|q_{\mathcal{O}^i}[k_{PC}]-v^{j}_{q_{\mathcal{O}^i}}[k_{PC}]|$.
...
According to the definition of PCA \cite{mackiewicz1993principal}, the data mapped to the principal component dimensions has little correlation. As a result, $Cov(Z^2_i, Z^2_j)$ ($1\leq i,j\leq D$) can be considered approximately as 0 because the series of random variables $\{Z^2_{k}\}$ is generated on the principal components. Therefore,
\begin{flalign}
Var(\frac{\sum^D_{k=1}Z^2_k}{\sqrt{\sum^{D}_{k=1}Var(Z^2_k)}})\approx1 \IEEEyesnumber \label{equ:independent}
\end{flalign}
...
Based on the notations above, the definition of $Z_k$, and the expression \ref{equ:normal_distribution_2}, \ref{equ:euclidean_distance}, and \ref{equ:independent}, the proposition needing to be proved is equivalent to the followings:
\begin{flalign}
...
\end{flalign}
,where $|a^2_k-\frac{\kappa_2(a^2_k+a_kb_k+b^2_k)}{3}|$ and $|b^2_k-\frac{\kappa_1(a^2_k+a_kb_k+b^2_k)}{3}|$ are assigned to $\eta_{1k}$ and $\eta_{2k}$, respectively.
\begin{comment}
\begin{flalign}
...
\begin{flalign}
\frac{|\dknn(q_1)-\dknn(q_2)|}{\max{(\dknn(q_1),\dknn(q_2))}}\leq \Delta  \IEEEyesnumber
...
\noindent\textit{Step 2.}
For each sampled data point in each group, the $\dknn$ values of the data points visited at the leaf nodes of the \hdrtree part of the \dualtree during its RkNN search process are divided into different sets. For any two $\dknn$ values $d_1$ and $d_2$ in the same set, the relationship below should hold that:
\begin{flalign}
\frac{|d_1-d_2|}{\max\{d_1,d_2\}}\leq \Delta \IEEEyesnumber
\end{flalign}
