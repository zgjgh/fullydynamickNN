...
The false-positive error in Example \ref{example:false positive error in kNN Search} arises because the point $i_r$, which determines the large cluster radius, is positioned on the far side relative to the query point $u_q$. The RkNN case follows the same mechanism: the radius-defining query point in an \hdrtree cluster may loosen the cluster-level bound with respect to $\dmknn(C_j)$. This spatial misalignment keeps the cluster-level lower bound correct but loose, leaving member points that can be pruned once point-to-point distances are examined.
...
Given the approximate function of $dist_{PC}^2(q,v)/dist^2(q,v)$, the remaining task is to determine the two parameters $k$ and $b$. Let $\rho_1=e_1/\sum_{i=1}^{D}e_i$. We fit $k\ln(d)+b$ by matching the two endpoints, namely $\rho(1)=\rho_1$ and $\rho(D)=1$. Since $\ln(1)=0$, the parameters are
\begin{equation}
...
The local distribution is approximately a Normal distribution, which can be written in mathematical form as $\frac{dist^2(q_{\mathcal{O}^i},v^{j}_{q_{\mathcal{O}^i}})}{b^2_{p}(q_{O^i},j)} \sim \mathcal{N}(\mu, \sigma^2), \, \text{approximately}$. 
