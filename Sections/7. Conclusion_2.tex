...
In the kNN search case, $q\in U$ and a leaf-level visited point $v\in I$ comes from the \deltatree part of \dualtree. The pruning bound $b_p$ is the farthest distance in the current candidate kNN list of $q$ (see the exact kNN search algorithm on \deltatree~\cite{cui2003contorting}), and $\mathcal{O}_U$ denotes the smallest $D$-dimensional space containing $U$ during the update process. To estimate the parameters of $f_Z$, we sample 10\% of the data points from each leaf node of the \hdrtree part of \dualtree, where the points in $U$ are stored. For each sampled point, the kNN search process records its $\dknn$ value, the number of distinct $b_p$ values encountered during the search, and the observed $Z$ values.
