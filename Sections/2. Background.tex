...
Let $\mathcal{X}\subseteq\mathbb{R}^D$ be a $D$-dimensional vector space, and let $dist:\mathcal{X}\times\mathcal{X}\rightarrow\mathbb{R}_{\ge 0}$ be a distance metric. In this paper we use Euclidean distance unless stated otherwise.
For any two points $a,b\in\mathcal{X}$, their Euclidean distance is $dist(a,b)=\lVert a-b\rVert_2=\sqrt{\sum_{j=1}^{D}(a[j]-b[j])^2}$.
...
\begin{definition}[$k$-th nearest-neighbor distance]\label{def:dknn}
Given $u$, $I$, and $k$, the $k$-th nearest-neighbor distance of $u$ over $I$ is
\[
...
This definition is important because our task is not to answer isolated nearest-neighbor queries on demand. Instead, we maintain a stored neighborhood relation whose rows must remain current as the underlying sets change. In that maintenance process, the operations defined above are invoked repeatedly: updates to $U$ or $I$ may require recomputing kNN results for some points, using RkNN search to identify affected rows, inserting or deleting rows of the materialized join table, and refreshing auxiliary index summaries.

\begin{definition}[Partially dynamic kNN join]\label{def:partial-dynamic-knnj}
Given $U$, $I$, and $k$, a partially dynamic kNN join maintains $\knnjoin(U,I)$ under updates to only one of the two sets while the other set remains fixed.
\end{definition}

\begin{definition}[Fully dynamic kNN join]\label{def:fully-dynamic-knnj}
Given $U$, $I$, and $k$, a fully dynamic kNN join maintains $\knnjoin(U,I)$ under interleaved insertions and deletions to both $U$ and $I$.
\end{definition}

\noindent\textbf{Problem statement.}
We study fully dynamic kNN join on high-dimensional data: maintain $\knnjoin(U,I)$ efficiently as both $U$ and $I$ are updated, with $D$ large enough that full-dimensional distance computation is a heavy cost. Throughout the paper, exact fully dynamic kNN join means that after each update the maintained join table is identical to the result obtained by recomputing the kNN join from scratch under the same tie-breaking rule.
...
\noindent\textbf{Maintenance of \deltatree and \hdrtree.}
