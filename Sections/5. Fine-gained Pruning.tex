...
In this section, we propose a novel pruning technique that can be used on \dualtree to improve its performance for \fdknnj. The new technique enhances \dualtree by providing pruning power to its leafnodes based on the low-dimensional distance between the query point and the data points in the leafnodes.   

Notably, we will present a theory to compute the optimal number of dimensions for the pruning distance that can achieve the balance of pruning efficiency and the time complexity of a single distance computation. The theory proposed is a comprehensive theory which covers a complete derivation from the measurement framework of the computation cost in leafnodes to the analysis of data distribution. 
...
%resulting in the retention of a number of data points that should be pruned for being far away from the query point, a condition known as "false positive".
%
...
\subsection{A step of point-level and low-dimensional distance based pruning}

As is discussed, whether kNN Search in the \deltatree part or RkNN Search in the \hdrtree part of the \dualtree, a considerable number of data points that are far away from the query points are still retained when the search progresses to the leafnodes. For those distant data points, full-dimensional distance computation used in the original search method on \dualtree introduces redundant computational overhead, because a low-dimensional distance is sufficient to identify such points in most cases. 

In response to such disadvantage brought by cluster-level pruning on the non-leaf levels of \dualtree, we come up with a new pruning step that is based on point-to-point low-dimensional distance computation. The new step can be integrated into the search algorithms on the \dualtree and applied at the leaf nodes to enhance search efficiency.  

We introduce the core theorem of this new step below.


\stitle{Theorem for the new pruning step}
\begin{theorem}{(Low dimensional point-wise pruning).}
Consider a data point $q$ whose $kNN$ or $RkNN$ list needs to be computed and a visited point $v$ located at a leaf node during the search process of $q$ on a \dualtree. Let $b_p$ denote $dist_{prune}$ when kNN Search and $dist_{kNN}(v)$ when RkNN Search. According to the definition of RkNN Search and the mechanism of kNN Search on \dualtree, $b_p$ serves as a pruning threshold such that $v$ is pruned if $dist(q, v) \geq b_p$, and retained for further evaluation otherwise. The pruning rule is then defined as follows:

\begin{align}
& \text{If } \mathrm{dist}_{PC}(q, v) \geq b_p,  \text{ then prune } v; \text{ otherwise, } \nonumber \\
& \text{compute } \mathrm{dist}(q, v) \text{ and accordingly update the kNN (RkNN)} \nonumber \\
&\text{list of q.} \nonumber
\end{align}
In the rule above, $dist_{PC}(q,v)$ means the low-dimensional version produced by PCA of the distance $dist(q,v)$ between $v$ and $q$ .

\begin{proof}
    If $dist_{PC}(v,q)\geq b_p$, then according to theorem \ref{thm:distance_pca}, 
    \begin{flalign}
        dist(q,v)\geq dist_{PC}(q,v)\geq b_p
    \end{flalign}
    therefore, $v$ can be pruned.
    
    Otherwise, the relative order between $dist(q,v)$ and $b_p$ can not be judged by $dist_{PC}(q, v)$, and $dist(q,v)$ should be calculated.
\end{proof}
\end{theorem}


This newly introduced step performs a low-dimensional distance computation for each visited data point at the leaf level of \dualtree, enabling early pruning of distant candidates at low cost before the full-dimensional distance is evaluated.
For the RkNN search algorithm on \dualtree (Algorithm \ref{alg:RKNNSearch}
), it should be inserted between lines 2 and 3, which handles the leaf nodes of the \hdrtree part. Similarly, for the kNN search algorithm (Algorithm \ref{alg:knnsearch}), the new step should be placed between lines 10 and 11, where it processes the leaf nodes of the \deltatree part.   



 This newly introduced pruning step can significantly enhance the search algorithm on \dualtree. Because a considerable data points in the leafnodes can be pruned by one time of low dimensional distance computation before the step of full-dimensional distance computation. Retained because of the insufficient pruning on the non-leaf levels of \dualtree, such data points are remote to the query point, therefore, even their distance to the query point on a few principle component dimensions can exceed the pruning bound, allowing them to be pruned by low-dimensional distance computation.
...
\subsection{Strategy for deriving the optimal pruning dimensionality}

The goal of selecting an optimal number of pruning dimensions is to minimize the expected computational cost for a randomly visited data point on leaf level of \dualtree during the search process for a randomly selected search-originating data point. To achieve this, we propose a theoretically grounded strategy to determine the optimal pruning dimensionality. This strategy first establishes a mathematical model that characterizes the relationship between the pruning dimensionality and the expected computational cost for a randomly selected visited point on leaf level. Based on this model, optimization methods are applied to derive the dimensionality that minimizes the expected cost.
...
\begin{flalign}
&\mathcal{O}_U=\{P|u[j]_{min}\leq P[j]\leq u[j]_{max}, u \in U, 1\leq j\leq D\}\\ \IEEEyesnumber
&\mathcal{O}_I=\{P|i[j]_{min}\leq P[j]\leq i[j]_{max}, i\in I, 1\leq j\leq D\}         \IEEEyesnumber
\end{flalign}
...
\begin{flalign}
P_p(d)&=P(dist_{PC}(q,v)\geq b_p)  \IEEEnonumber \\
&=P(\frac{dist^2(q,v)}{b_p^2}\geq \frac{dist^2(q,v)}{dist_{PC}^2(q,v)})  \IEEEyesnumber
\label{equ: ped_basic}
\end{flalign}
...
\ssstitle{Distance Shrinkage and Pruning Dimensionality}:

To investigate the functional relation between the pruning dimensionality and the square rate $dist^2(q,v)/dist^2_{PC}(q,v)$ between the full-dimensional distance and the low-dimensional distance computed on a part of principle component dimensions, which is the distance shrinkage effect of dimensional reduction by PCA, we revisit the properties of PCA and observe that, the eigenvalue $e_i$ of the co-variance matrix $M$ of the initial state of $U\cup I$ corresponding to the $i$th row vector of the PC matrix $M_{PC}$ of $M$ is equal to the variance $\sigma^2_i$ of the elements of the data points in initial $U\cup I$ along the $i$th dimension of the PC space. And $\sigma^2_i$, defined as $\frac{\sum_{p\in U\cup I}(p[i]-\overline{p[i]})^2}{|U\cup I|}$, represents the degree of dispersion of the data points along such dimension. Consequently, given the pruning dimensionality $d$ and the number of full dimensions $D$, the following approximation can be made reasonably that:
\begin{flalign}
    \frac{dist^2(v,q)}{dist_{PC}^2(v,q)}\approx\frac{\sum_{i=1}^{D}\sigma^2_i}{\sum_{i=1}^{d}\sigma^2_i}= \frac{\sum_{i=1}^{D}e_i}{\sum_{i=1}^{d}e_i} \yesnumber
\end{flalign}

Because the row vectors in the PC matrix are sorted in descending order according to their corresponding eigenvalues, the speed of increasing of $ \frac{\sum_{i=1}^{d}e_i}{\sum_{i=1}^{D}e_i}$ is fast when $d$ is small and turn slower as $d$ increases, forming a curve that rises sharply first and tends to be flat in the coordinate system whose X-axis represents $d$ and Y-axis represents $ \frac{\sum_{i=1}^{d}e_i}{\sum_{i=1}^{D}e_i}$. Therefore, a function that $\frac{\sum_{i=1}^{d}e_i}{\sum_{i=1}^{D}e_i}=kln(d)+b$, which forms a similar function graph, can be used to approximately estimate the relationship between $ \frac{\sum_{i=1}^{d}e_i}{\sum_{i=1}^{D}e_i}$ and $d$. As a result, the functional model $kln(d)+b$ can also be applied for $dist^2(q,v)/dist^2_{PC}(q,v)$, which can be written in the following mathematical form that
\begin{flalign}
    \frac{dist^2(v,q)}{dist_{PC}^2(v,q)}\approx \frac{\sum_{i=1}^{D}e_i}{\sum_{i=1}^{d}e_i}\approx kln(d)+b \yesnumber
\end{flalign}

Given the approxiamte functional model of $dist^2(q,v)/dist^2_{PC}(q,v)$, what remains to achieve are the two parameters $k$ and $b$. Parameters $k$ and $b$ can be located by the following two pairs of values of $d$ and $\frac{\sum_{i=1}^{d}e_i}{\sum_{i=1}^{D}e_i}$, that is:
\begin{flalign}
...
Above we present the process of confirm the relation between the pruning dimensionality and the square rate $dist^2(q,v)/dist^2_{PC}(q,v)$ between the full-dimensional distance $dist^2(q,v)$ and low-dimensional distance $dist^2_{PC}(q,v)$ of a randomly selected search-originating data point $q$ and visited data point $v$. In order to achieve the function between pruning dimensionality and the pruning probability, the global distribution will be discussed in what follows. 

\ssstitle{Global Distribution} 

We use the technique of statistic inference, which is the commonly used approach to approximate the global distribution. Formulation of $f_{dist^2(q,v)/b^2_p}$ can be accomplished by two steps, which are deriving the probability model of $dist^2(q,v)/b^2_p$ and confirming the parameters in the probability model.


\sssstitle{Model of global distribution}:According to the law of total probability and Bayes' Theorem and the conditions assumed before, the theorem  below can be obtained.
\begin{theorem}{(Model of global distribution).}
Global distribution is a linear combination of local distributions, in mathematical form, it can be written as what follows:
\begin{flalign}
f_{dist^2(q,v)/b^2_p}\approx \sum^{M}_{i=1}\sum^{B_i}_{j=1} P_i\pi_{ij}f_{ij}   \IEEEyesnumber
\label{equ: F_general_framework}
\end{flalign}
\end{theorem}
\label{thm:framework_F}
\begin{proof}
Because of the Law of Total Probability and the property 1 of the condition 3 of a stable phase assumed before, the approximation below holds throughout the entire stable phase:
\begin{flalign}
f_{dist^2(q,v)/b^2_p}&=\sum^{M}_{i=1}P(q\in \mathcal{O}^i)f_{(dist^2(q,v)/b^2_p|q\in \mathcal{O}^i)} \IEEEnonumber   \\
&\approx \sum^{M}_{i=1}P_if_{(dist^2(q,v)/b^2_p|q\in \mathcal{O}^i)}  \IEEEyesnumber
\label{equ:F_general_domain_partiton}
\end{flalign}

Because of the Law of Total Probability and the property 2,3,4 of the condition 3 assumed, the below approximation holds throughout the entire stable phase.
\begin{flalign}
&f_{(dist^2(q,v)/b^2_p|q\in \mathcal{O}^i)} \IEEEnonumber \\
&=\sum^{B_i}_{j=1} P(b_p=b_p(q,j)|q\in \mathcal{O}^i)f_{(dist^2(q,v)/b^2_p|b_p=b_p(q,j);q\in \mathcal{O}^i)}  \IEEEnonumber  \\
&\approx \sum^{B_i}_{j=1} \pi_{ij}f_{ij} \IEEEyesnumber
\label{equ:F_general_pruningboundpartition}
\end{flalign}
Therefore, $f_{dist^2(q,v)/b^2_p}\approx \sum^{M}_{i=1}\sum^{B_i}_{j=1} P_i\pi_{ij}f_{ij}$ holds during the whole stable phase.
\end{proof}
...
According to the property 4 of condition 3 of a stable phase, $f_{dist^2(q_{\mathcal{O}^i},v^{j}_{q_{\mathcal{O}_i}})/b^{2}_{p}(q_{O^i},j)} \approx f_{ij}$ regardless of the specific position of $q_{\mathcal{O}^i}$ in $\mathcal{O}^i$. 

Therefore, $f_{dist^2(q_{\mathcal{O}^i},v^j_{q_{O^i}})/b^{2}_p(q_{O^i},j)}$, which is a local distribution where $q_{\mathcal{O}^i}$ is an arbitrary data point in $\mathcal{O}_i$ but treated as a fixed point in the process of producing $dist^2(q_{\mathcal{O}^i},v^{j}_{q_{\mathcal{O}^i}})$ while $v^{j}_{q_{\mathcal{O}^i}}$ is a random visited point encountering the $j$-th pruning bound (bound kernel) value of the search of $q_{\mathcal{O}^i}$, can be analyzed to formulate the model of the global distribution that is the distribution of $dist^2(q,v)/b^2_p$.

We propose the following theorem:


\begin{theorem}{(Local distribution.)}
Local distribution is approximately a Normal Distribution, which can be written in mathematical form as below:
\begin{flalign}
\frac{dist^2(q_{\mathcal{O}^i},v^{j}_{q_{\mathcal{O}^i}})}{b^2_{p}(q_{O^i},j)} \sim \mathcal{N}(\mu, \sigma^2), \quad \text{approximately} \IEEEyesnumber
\end{flalign}
\label{thm:normal_distribution_local}
\end{theorem}
The proof for theorem \ref{thm:normal_distribution_local} is presented in Appendix A.
...
\begin{flalign}
f_{dist^2(v,q)/b_{p}^2}(x)\approx& \sum^{T}_{m=1}p_mf_m\nonumber \\=& \sum^{T}_{m=1}p_m\times \frac{1}{\sqrt{2\pi\sigma^2_m}}\exp \left(-\frac{(x-\mu_m)^2}{2\sigma^2_m}\right) \IEEEyesnumber
\end{flalign}
where $T=\sum^{M}_{i=1}B_i$, and $f_{m}(x)$ is the probability density function of a Normal Distribution $N(\mu_m, \sigma^2_m)$.

After formulation of the distribution model of $dist^2(q,v)/b_{p}^2$, the parameters in the model should be further estimated to derive the concrete $f_{dist^2(q,v)/b_{p}^2}(x)$.

\sssstitle{Parameters in \bm{$f_{dist^2(q,v)/b_{p}^2}$}}: At the beginning of a new stable phase, we employ a sampling-based method to determine the parameters in the density distribution function $f_{dist^2(q,v)/b_{p}^2}$ of $dist^2(q,v)/b_{p}^2$ throughout the stable phase. Since the data distribution evolves slowly during the stable phase, it is reasonable to assume that the data in $U\cup I$ at the beginning of the phase remains representative of the entire period. Thus, estimating the parameters of $f_{dist^2(q,v)/b^2_{p}}$ based on the initial state of the stable phase is reasonable.
...
In the scenario of kNN Search, the query point $q$ is from $U$, $v$, which is a data point visited at a leafnode of the \deltatree part of the \dualtree is from $I$, $b_p$ represents the distance from $q$ to the farthest point in its current candidate kNN list (refer to the algorithm \ref{alg:knnsearch} for kNN Search for more details), and $\mathcal{O}_U$ denotes the smallest $D$-dimensional space containing all data points in $U$ during the stable phase. 
...
The discussion above discussed our strategy for estimating the parameters of $f_{dist^2(q,v)/b^2_p}$ during the stable phase, based on the condition that data distribution changes slowly within a stable phase and by analyzing samples taken from its initial state. 

At this stage of the paper, we have derived the relationship between the pruning dimensionality $d$ and the square rate $dist^2(v,q)/dist^2_{PC}(v,q)$ between the full-dimensional distance $dist^2(q,v)$ and the low-dimensional distance $dist^2_{PC}(q,v)$ computed on a front part of principle component dimensions of the search-originating point $q$ and the visited point $v$. And the approximate global distribution which is the approximate distribution of $dist^2(q,v)/b^2_p$ also has been derived. Following this, the function for the pruning probability $P_{p}(d)$ can be obtained in the following deduction:
...
&\approx \frac{-kD}{d(kln(d)+b)^2}\sum^{T}_{m=1}\frac{p_m}{\sqrt{2\pi \sigma^2_m}}\exp\left(-\frac{(\frac{1}{kln(d)+b}-\mu_m)^2}{2\sigma^2_{m}}\right) \nonumber\\
&+1 \yesnumber
\end{flalign}

Let $\frac{\mathrm{d}E_{cost(q,v)}(d)}{\mathrm{d}d}=0$, we have 
\begin{flalign}
&\frac{kD}{d(kln(d)+b)^2}\sum^{T}_{m=1}\frac{p_m}{\sqrt{2\pi \sigma^2_m}}\exp\left(-\frac{(\frac{1}{kln(d)+b}-\mu_m)^2}{2\sigma^2_{m}}\right) \nonumber \\
&=1 \yesnumber
\end{flalign}
...
\leveltwo{For the simplified pruning probability function}

Based on the results discussed earlier in expression \ref{equ:simppd}, the derivative of $E_{cost(v,q)}(d)$ with respect to $d$ can be provided as below:

\begin{flalign}
    \frac{\mathrm{d}E_{cost(q,v)}(d)}{\mathrm{d}d}=c(-D\cdot \frac{k''}{d}+1)
\end{flalign}
Then, a theorem can be established that
\begin{theorem}
    When the pruning dimensionality $d$ is equal to $Dk''$ where $D$ is the full dimensionality and $k''$ is the slope paramter of the simplified prunning probability function that $P_{p}(d)=k''d+b$, then, $E_{cost(q,v)}(d)$ approximately achieve its minimal value.
    \begin{proof}
        When $d=Dk''$, it can be verified that 
        \begin{flalign}
       \frac{\mathrm{d}E_{cost(q,v)}(d)}{\mathrm{d}d}=0 \yesnumber
        \end{flalign}
        When $d<Dk''$, it can be verified that
        \begin{flalign}
       \frac{\mathrm{d}E_{cost(q,v)}(d)}{\mathrm{d}d}<0 \yesnumber
        ...
        When $d>k''D$, it can be verified that
        \begin{flalign}
       \frac{\mathrm{d}E_{cost(q,v)}(d)}{\mathrm{d}d}>0 \yesnumber
        \end{flalign}
        because $k''>0$
