...
Algorithm~\ref{alg:UpdateHDRInsert} updates the selected path immediately. If insertion extends a branch beyond the height estimated at construction, every additional level uses the full dimensionality $D$. Let $T_{\mathrm{proj}}$ and $T_{\mathrm{grow}}$ denote projection and optional subtree-growth costs, including center recomputation during repartitioning. Evaluating at most $f$ children per level gives $O(T_{\mathrm{proj}}+f\sum_{l=1}^{L}d_l+T_{\mathrm{grow}})$, with the last term absent when no overflow occurs.

\noindent\textbf{Direct update links.}
Our extension replaces the recursive path scan sketched for the original \hdrtree~\cite{yang2014continuous} by adding a direct query-to-leaf pointer and a parent link to each tree node. Without these links, every deletion or refresh first searches from the root, and the scan may visit unrelated branches. Even a favorable single-path traversal examines up to $f$ children per level, requiring $O(Lf)$ child tests and $O(f\sum_{l=1}^{L}d_l)$ reduced-space work when geometric tests are used. The links instead locate the leaf in $O(1)$ and recover its ancestors in $O(L)$, reducing child inspection by up to a factor of $f$ and eliminating geometric path-search work. Without them, a reference update repeats this lookup for every affected query. Construction and structural updates maintain the links, whose additional space is $O(|U|+N_H)$ for $N_H$ tree nodes.

\noindent\textbf{Query deletion.}
The leaf pointer first locates a deleted query $u$. Its point count, radius, and $\dmknn$ are then updated along the parent path. A leaf radius is recomputed from at most $\theta$ resident queries. At a non-leaf cluster, a valid enclosing radius is tightened from at most $f$ child bounding summaries in the current PCA space. This upper bound preserves the cluster-level pruning condition without scanning the whole subtree. If a non-leaf subtree falls below $\theta$ queries, it is collapsed into a full-dimensional leaf and the remaining query pointers are redirected to that leaf.
...
Only the ancestor path of $u$ changes. Without collapse, the leaf and ancestor-summary costs are $O(\theta D)$ and $O(f\sum_{l=1}^{L}d_l)$, respectively. Path-wise collapses add at most $O(L\theta D)$ work.

\noindent\textbf{Pruning-parameter refresh.}
An update to $I$ leaves the query geometry unchanged. It may alter the stored kNN distance $\dknn(u,I)$. This changes the maximum distance recorded on its ancestor path. The leaf pointer locates $u$ directly; we refresh its distance and propagate $\dmknn$ upward from leaf entries and immediate-child parameters.
...
Only ancestors of $u$ can change, and propagation stops once a recomputed maximum is unchanged. If $h\leq L$ levels are visited, Algorithm~\ref{alg:UpdateHDRRefresh} costs $O(\theta+hf)$, or $O(\theta+Lf)$ in the worst case, using only scalar maximum updates and no high-dimensional distance computation.
