...
To support efficient kNN and RkNN search in the two directions required by fully dynamic kNN join, \dualtree maintains a \deltatree over the reference set $I$ and an \hdrtree over the query set $U$. The underlying exact search procedures follow the original \deltatree and \hdrtree algorithms~\cite{cui2003contorting,yang2014continuous}. This dual-index system must itself remain current as both inputs evolve. The \deltatree already supports insertions and deletions on its indexed reference set, whereas the original \hdrtree assumes a static indexed query set. We therefore extend \hdrtree with query insertion and deletion, together with local refresh of its query-dependent pruning values when reference updates change maintained kNN rows. We first present this extension and then describe how updates to $U$ and $I$ coordinate the maintenance of both the materialized kNN join table and the dual-index state.

\subsection{Dynamic Extension of \hdrtree}
\label{subsec:dynamic-hdr}

The query-side index changes in two ways under a fully dynamic workload. An update to $U$ changes its indexed membership and spatial structure, whereas an update to $I$ may change the stored kNN distances of affected queries and hence the pruning values derived from them. Our extension handles both structural changes and query-dependent pruning-state changes, allowing \hdrtree to remain usable throughout updates to either input.
...
\noindent\textbf{Deletion and threshold refresh.}
Each query maintains a direct pointer to its hosting leaf entry together with its ancestor path in \hdrtree. When a query is deleted or its stored kNN distance changes after a reference update, these pointers locate the affected path without searching unrelated subtrees. The resulting change is then propagated from the hosting leaf toward the root, restoring the maximum kNN-distance value of each ancestor cluster; query deletion may additionally tighten the cluster radius.
...
\ForEach{$C\in P$ from leaf to root}{
    recompute $\dmknn(C)$\;
    \If{mode = delete}{
...
Algorithm~\ref{alg:UpdateHDRAdjust} makes query-side maintenance path-local. In particular, when a reference update changes $\dknn(u,I)$, pointer-guided bottom-up propagation restores the exact query-dependent threshold of every affected ancestor cluster without scanning unrelated clusters to rediscover $u$.
