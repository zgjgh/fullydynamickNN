...
Our extension adds two links that are absent from the original \hdrtree~\cite{yang2014continuous}. Each query entry points directly to the leaf that currently contains it. Each non-root tree node points to its immediate parent. The query-to-leaf pointer exposes the query's physical location, while the parent links expose exactly the ancestor path whose structure or pruning parameters may change. Tree construction and structural updates maintain the links. Their additional space is $O(|U|+N_H)$ for $N_H$ tree nodes.

\noindent\textbf{Query deletion.}
For deletion, the query-to-leaf pointer identifies the leaf containing $u$, and the parent links delimit the only ancestor path that can change. Algorithm~\ref{alg:UpdateHDRDelete} gives the complete procedure.
...
Algorithm~\ref{alg:UpdateHDRDelete} obtains the hosting cluster from $leaf(u)$ and deletes $u$ from that leaf (Lines~1--2). It then follows the parent links upward (Lines~3--10). At each cluster, it recomputes the point count, collapses an underfull subtree and redirects its leaf pointers when necessary, tightens the radius, and recomputes $\dmknn$. A leaf radius uses at most $\theta$ resident queries, whereas a non-leaf radius uses $f$ child bounds without scanning the entire subtree.

Without these links, deletion must first recover the hosting path from the root and may inspect unrelated branches. Even a favorable traversal restricted to one path requires $O(Lf)$ child tests and $O(f\sum_{l=1}^{L}d_l)$ reduced-space work when geometric tests are used. With the links, the leaf is located in $O(1)$ and its ancestors are recovered in $O(L)$. Since $f$ is fixed, the child-test saving is a constant factor, while the geometric path-search work is eliminated. After location, the leaf and ancestor-summary costs are $O(\theta D)$ and $O(f\sum_{l=1}^{L}d_l)$, respectively. Path-wise collapses add at most $O(L\theta D)$ work.

\noindent\textbf{Pruning-parameter refresh.}
An update to $I$ leaves the query geometry unchanged but may alter the stored kNN distance $\dknn(u,I)$. Algorithm~\ref{alg:UpdateHDRRefresh} refreshes the resulting query-dependent pruning parameters.
...
Algorithm~\ref{alg:UpdateHDRRefresh} uses $leaf(u)$ to obtain the hosting cluster and refreshes $\dknn(u,I)$ (Lines~1--2). At each visited cluster, it stores the old $\dmknn$, recomputes the value from the leaf entries or immediate-child parameters, and moves to the parent only if the maximum changes (Lines~3--8). If it is unchanged, no higher ancestor can change and propagation stops.

Without the links, the root-to-leaf scan may inspect unrelated branches, and its geometric path tests would be repeated for every affected query after a reference update. With the links, location is $O(1)$. If $h\leq L$ levels are visited, the bottom-up refresh costs $O(\theta+hf)$, or $O(\theta+Lf)$ in the worst case. It uses only scalar maximum updates and no high-dimensional distance computation.
