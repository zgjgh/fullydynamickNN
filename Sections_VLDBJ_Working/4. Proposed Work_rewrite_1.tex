...
Without the links, every affected query first incurs $O(Lf)$ membership tests to recover its root-to-leaf path before bottom-up propagation begins. The total refresh cost is therefore $O(Lf+\theta+hf)$. With $O(1)$ leaf access, the links reduce this cost to $O(\theta+hf)=O(\theta+h)$ for fixed $f$, or $O(\theta+L)$ in the worst case. Since $h$ is ordinarily comparable to $L$, the gain comes from removing one complete top-down membership scan for every affected query. The refresh itself uses only scalar maximum updates and no high-dimensional distance computation.
