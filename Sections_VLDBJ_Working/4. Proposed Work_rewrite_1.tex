...
Algorithm~\ref{alg:UpdateHDRInsert} updates the selected path immediately. If insertion extends a branch beyond the height estimated at construction, every additional level uses the full dimensionality $D$. Let $T_{\mathrm{proj}}$ and $T_{\mathrm{grow}}$ denote projection and optional subtree-growth costs. Evaluating at most $f$ children per level gives $O(T_{\mathrm{proj}}+f\sum_{l=1}^{L}d_l+T_{\mathrm{grow}})$, with the last term absent when no overflow occurs.

\noindent\textbf{Direct update links.}
To replace the recursive path scan sketched for the original \hdrtree~\cite{yang2014continuous}, our extension adds a direct query-to-leaf pointer and a parent link to each tree node. Construction, insertion, subtree growth, and collapse maintain these links. A top-down path search may inspect up to $f$ children per level, costing $O(Lf)$ child tests and, when geometric tests are needed, $O(f\sum_{l=1}^{L}d_l)$ reduced-space work. The new links instead locate the leaf in $O(1)$ and recover its ancestors in $O(L)$, reducing path-localization cost by up to a factor of $f$. Their additional space is $O(|U|+N_H)$ for $N_H$ tree nodes.

\noindent\textbf{Query deletion.}
The leaf pointer first locates a deleted query $u$. Its point count, radius, and $\dmknn$ are then updated along the parent path. If a non-leaf subtree falls below $\theta$ queries, it is collapsed into a full-dimensional leaf and the remaining query pointers are redirected to that leaf.
