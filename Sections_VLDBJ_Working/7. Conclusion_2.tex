...
\label{equ:tail_term_expectation}
\end{equation}
\begin{lemma}[Tail term]\label{lm:first_item}
For $\epsilon=(5\xi/4)^{1/2}$, $t^2T_\epsilon/s_D^2=0$.
\begin{proof}
...
\label{equ:zero_1}
\end{align}
Since $\delta$ is arbitrary, letting $\delta\to+\infty$ in \eqref{equ:zero_1} yields $\frac{t^2T_\epsilon}{s_D^2}=0$, proving the lemma.
\end{proof}
\end{lemma}
...
\]
\begin{proof}
Let $k^\star=\arg\max_{1\leq k\leq D}Var(Y_k)$. Since $Y_k$ is centered, $Var(Y_{k^\star})=E(Y_{k^\star}^2)$. Splitting this expectation at $\epsilon s_D$ and using Lemma~\ref{lm:first_item}, we have
\begin{align}
t^4\max_{1\leq k\leq D}\frac{Var(Y_k)}{s_D^2}
&\leq t^4\left(\epsilon^2+\frac{T_\epsilon}{s_D^2}\right)
=t^4\epsilon^2
=\frac{5}{4}\xi t^4 .
\label{equ:sec_term}
\end{align}
...
\end{proof}
\end{lemma}
With Lemma~\ref{lm:xi_zero}, the characteristic-function bound in \eqref{equ:new_bound} can be transferred to the distribution functions through Esseen's inequality.
...
In the kNN search case, $q\in U$ and a leaf-level visited point $v\in I$ comes from the \deltatree part of \dualtree. The pruning bound $b_p$ is the farthest distance in the current candidate kNN list of $q$, and $\mathcal{O}_U$ denotes the smallest $D$-dimensional space containing $U$ during the update process. To estimate the parameters of $f_Z$, a specified proportion of the data points is sampled from each leaf node of the \hdrtree part of \dualtree, where the points in $U$ are stored; this paper uses 10\%. For each sampled point, the kNN search process records its $\dknn$ value, the number of distinct $b_p$ values encountered during the search, and the observed $Z$ values.
...
\end{flalign}
where $q_1$ and $q_2$ are arbitrary two data points in a group. The number of groups formed serves as an approximation for $M$. The threshold ratio $\Delta$ can be specified as needed; this paper uses 10\%.
...
After determining $M$ and $B_i$, the observed $Z$ values from the kNN search traces, which are collected during sampling but not used to estimate $M$ and $B_i$, are used to fit the Gaussian mixture model for $f_Z$. We apply the standard EM algorithm \cite{ng2012algorithm} to estimate the mixture weights $p_m$ and Gaussian parameters $\mu_m$ and $\sigma_m$.

\noindent\textbf{Parameters for RkNN search.}
In the RkNN search case, $q\in I$ and a leaf-level visited point $v\in U$ comes from the \hdrtree\ part of \dualtree. The pruning bound $b_p$ is $\dknn(v,I)$, and $\mathcal{O}_I$ denotes the smallest $D$-dimensional space containing $I$ during the update process. 

To estimate the parameters of the RkNN-side $f_Z$, a specified proportion of the data points is sampled from each leaf node of the \deltatree part of \dualtree, where the points in $I$ are stored; this paper uses 10\%. For each sampled point, the RkNN search process records the $\dknn$ values of the visited leaf-level query points in the \hdrtree part of \dualtree and the observed $Z$ values.
...
\end{flalign}
where $\Delta$ is a small threshold ratio that controls the within-set variation of the $\dknn$ values under a low level; this study uses $\Delta=10\%$. For each sampled data point, the number of sets gained is recorded. Then, the mode of these set counts across all the sampled data points in the $i$-th group can serve as the approximation of the number $B_i$ of bound kernels of the $i$-th subspace $\mathcal{O}^{i}_I$ of $\mathcal{O}_I$. 

Finally, as in the kNN search case, the sampled RkNN-side $Z$ values are fitted by the standard EM algorithm \cite{ng2012algorithm} to estimate the mixture weights $p_m$ and Gaussian parameters $\mu_m$ and $\sigma_m$.
